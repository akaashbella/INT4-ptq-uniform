#!/bin/bash
#SBATCH -J ptq-tin-eval-fp32-cmp
#SBATCH -p dgxh
#SBATCH --gres=gpu:1
...
# FP32 eval for compression runs (requires trained checkpoint in OUTPUT_ROOT).
...
echo "Starting EVAL FP32 compression: model=${model} regime=${regime} seed=${seed}"
