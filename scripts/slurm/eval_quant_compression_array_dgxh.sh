#!/bin/bash
#SBATCH -J ptq-tin-eval-quant-cmp
#SBATCH -p dgxh
#SBATCH --gres=gpu:1
...
# Weight-only PTQ eval (w8/w6/w4) for compression — needs eval_fp32.json in the run dir.
...
echo "Starting EVAL QUANT compression: model=${model} regime=${regime} seed=${seed}"
